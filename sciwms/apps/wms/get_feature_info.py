...
from ...util import cf, get_pyproj, get_rtree_nodes_path, rtree_nodes_exists, print_exception
...
def getFeatureInfo(request, dataset):
    ...
    try:
        ...
        #tlon, tlat = mi(xmin+((xmax-xmin)*(X/width)), ymax-((ymax-ymin)*(Y/height)), inverse=True)
        #lonmin, latmin = mi(xmin, ymin, inverse=True)
        #lonmax, latmax = mi(xmax, ymax, inverse=True)
        CRS = get_pyproj(request)
        tlon, tlat = pyproj.transform(CRS, EPSG4326, xmin+((xmax-xmin)*(X/width)), ymax-((ymax-ymin)*(Y/height)))
        lonmin, latmin = pyproj.transform(CRS, EPSG4326, xmin, ymin)
        lonmax, latmax = pyproj.transform(CRS, EPSG4326, xmax, ymax)

    except:
        exc_type, exc_value, exc_traceback = sys.exc_info()
        logger.error("getFeatureInfo ERROR: " + repr(traceback.format_exception(exc_type, exc_value, exc_traceback)))
